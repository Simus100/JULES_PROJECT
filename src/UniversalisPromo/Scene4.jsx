...
const Stat = ({ number, text, delay }) => {
  ...
  return (
    <div
      className="relative flex flex-col items-center justify-center p-16 bg-gradient-to-b from-gray-900/80 to-black/80 rounded-[3rem] border border-gray-700/50 backdrop-blur-md overflow-hidden"
      style={{
        transform: `scale(${scale})`,
        opacity: scale,
        boxShadow: `0 0 ${interpolate(scale, [0, 1], [0, 100])}px rgba(147, 51, 234, 0.3)`
      }}
    >
      {/* Dynamic glow behind text */}
      <div className="absolute inset-0 bg-gradient-to-tr from-blue-600/20 to-purple-600/20 blur-2xl" />

      <div className="z-10 text-9xl font-black text-transparent bg-clip-text bg-gradient-to-br from-blue-300 via-purple-400 to-pink-500 mb-6 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
        {number.includes("+") ? `${numValue}+` : number.includes("%") ? `${numValue}%` : numValue}
      </div>
      <div className="z-10 text-3xl text-gray-200 font-bold uppercase tracking-[0.2em] text-center">{text}</div>
    </div>
  );
};
...
export const Scene4 = () => {
  ...
  return (
    <AbsoluteFill className="bg-black flex flex-col items-center justify-center p-24 overflow-hidden">
      ...
      <div className="z-10 grid grid-cols-2 gap-24 w-full max-w-6xl mx-auto perspective-1000">
